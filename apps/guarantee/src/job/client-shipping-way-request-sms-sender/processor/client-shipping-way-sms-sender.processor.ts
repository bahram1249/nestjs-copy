...
export class ClientShippingWaySmsSenderProcessor extends WorkerHost {
  ...
  async process(job: Job<any, any, any>, token?: string): Promise<any> {
    try {
      const template = `${job.data.firstname} ${job.data.lastname} عزیز، لطفا با مراجعه به پنل کاربری خود نحوه ارسال کالا به آدرس مشخص شده را اعلام فرمایید و کالای خود را به نماینده ارسال فرمایید.`;
